...
class Group extends Backend
{
    ...
    public function roletree()
    {
        ...
        if (($pid || $parentGroupModel) && (!$id || $currentGroupModel)) {
            ...
            //角色组列表
            $groupTree->init(collection(model('AuthGroup')->where(['status'=>'normal'])->where('id', 'in', $this->childrenGroupIds)->select())->toArray());
            ...
            if (!$id || !in_array($pid, $this->childrenGroupIds) || !in_array($pid, $groupTree->getChildrenIds($id, true))) {
                ...
                $parentRuleIds = array_map(function ($item) {  //array_map:将函数作用到数组中的每个值上，每个值都乘以本身，并返回带有新值的数组
                    return $item['id'];
                }, $parentRuleList);
                $nodeList = [];
                foreach ($parentRuleList as $k => $v) {
