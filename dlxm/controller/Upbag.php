...
use app\admin\model\UpbagProductProp;
 ...
 * @icon fa fa-circle-o
 */
class Upbag extends Backend
{

    /**
     * Upbag模型对象
     * @var \app\admin\model\Upbag
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\Upbag;
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax())
        {
            ...
            foreach ($list as $row) {
                $row->visible(['id','name','price','list_image','inventory','has_prop','grade','weigh','status']);
                $row->visible(['product']);
                $row->getRelation('product')->visible(['weight']);
        ...
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                try {
                    ...
                    unset($params['grade_type']);
                    $result = $this->model->allowField(true)->create($params);
                    if($params['has_prop']){

                        $propData = $this->common->packPropData(['upbag_id'=>$result->id]);
                        UpbagProductProp::insertAll($propData) or $result = false;
                    }
                    if ($result) {
                        $this->model->commit();
                        $this->success();
...
//        halt(collection(Prop::getListLv())->toArray());
    ...
    public function edit($ids = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                try {
                    ...
                    $where['upbag_id'] = $row['id'];

                    //原先设置有属性，当前保存没属性
                    if($init_has_prop==1 && $params['has_prop']==0){
                        UpbagProductProp::where($where)->delete() or $result = false;
                    }
                    ...
                    if($params['has_prop']){
                        ...
                        $ids = array_flip(UpbagProductProp::where($where)->column('id'));
                        foreach ($propData as $v){
                            $where['props'] = $v['props'];
                            if($id = UpbagProductProp::where($where)->value('id')){
                                UpbagProductProp::where('id',$id)->update($v)!==false or $result = false;
                                unset($ids[$id]);
                            }else{
                                UpbagProductProp::create($v) or $result = false;
                            }
                        }
                        if(!empty($ids)){
                            UpbagProductProp::whereIn('id',array_flip($ids))->delete() or $result = false;
                        }
        ...
        list($product_prop_list,$prop_ids) = UpbagProductProp::product_props($row['id']);
