        ...
        try {
            ...
            foreach ($columnList as $k => $v) {
                ...
                if ($v['COLUMN_KEY'] != 'PRI' && !in_array($field, $this->reservedField) && !in_array($field, $this->ignoreFields)) {
                    ...
                    // 如果是number类型时增加一个步长  str_repeat() 函数把字符串重复指定的次数
...
EOD;
     ...
     * @param string $name
     * @return string
     * @param  str_replace() 函数以其他字符替换字符串中的一些字符（区分大小写）
     */
    ...
    protected function getArrayString($arr)
    {
        ...
        $stringArr = [];
        foreach ($arr as $k => $v) {
            $is_var = in_array(substr($v, 0, 1), ['$', '_']); //substr() 函数返回字符串的一部分;
            if (!$is_var) {
                $v = str_replace("'", "\'", $v);
                $k = str_replace("'", "\'", $k);
            }
    ...
    protected function getItemArray($item, $field, $comment)
    {
        ...
        if (stripos($comment, ':') !== false && stripos($comment, ',') && stripos($comment, '=') !== false) {
            ...
            foreach ($item as $k => $v) {
                $itemArr[$v] = is_numeric($v) ? $field . ' ' . $v : $v; //is_numeric() 函数用于检测变量是否为数字或数字字符串
            }
        }
        return $itemArr;
    }
